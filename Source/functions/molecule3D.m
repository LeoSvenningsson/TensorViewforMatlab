...
%
%
%   Version: 1.2/TensorView for Matlab
%   Author:     André Ludwig (mail@andreludwig.ch/aludwig@alumni.ethz.ch)
%               mathworks.com/matlabcentral/fileexchange/55231-molecule3d
%   Modifications by Leo Svenningsson (leo.svenningsson@chalmers.se)
%
% geometry settings
...
NB = 50; % bonds, more looks smoother
...
filetype=varargin{4+offs};
Conlist=varargin{5+offs};
lab = varargin{2+offs};
DBOND = varargin{6+offs}; % maximal distance of atoms forming bond, 1.6 Å is usually ok
if ~iscellstr(lab)
...
if filetype == ".pdb"
    ...
    if Conlist(1,1) ~= 0
        pairs = [];
        Conlist =Conlist(:,2:end);
        for i=1:length(Conlist(:,1))
            Cleancon= Conlist == i;
            Conlist(Cleancon) = 0;
            plist = Conlist(i,:) ~= 0;
            pairs = [pairs; [Conlist(i,plist)',zeros(length(Conlist(i,plist)),1) + i]];
        end
        ks = 1:length(pairs(:,1));
        fliplr(pairs);
        ds = sqrt(sum((xyz(pairs(:,1),:) - xyz(pairs(:,2),:)).^2,2));
...
for k = ks % draw sticks for all bounds
    ...
    rotZphi = [cos(phi) -sin(phi) 0; sin(phi) cos(phi) 0; 0 0 1];
    rotYtheta = [cos(theta) 0 sin(theta); 0 1 0; -sin(theta) 0 cos(theta)];
    
    % rotate cylinders to match bond vector v
    for kk = 1:numel(x)
        vr = [x(kk); y(kk); z(kk);];
        vr = rotZphi*rotYtheta*vr;
        x(kk) = vr(1);
        y(kk) = vr(2);
        z(kk) = vr(3);
